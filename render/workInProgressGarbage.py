...
#define SCAN_BLOCK_DIM 1024  // or whatever block size you're using
...
__global__ void manat_render_circles_sectioned() {
    ...
    for (int count = 0; count < numCircles; count++) {
        bool circlePresent = deviceSectionCircleLists[sectionNumber * numCircles + count];
        if (circlePresent) {
            int index3 = 3 * count;
            ...
            if (circleInBox(p.x, p.y, rad, boxL, boxR, boxT, boxB)) {
                shadePixel(co, pixelCenterNorm, p, imgPtr);
            }
        }
...
__global__ void prerender_circles() {
    ...
    for (int i = startCircle; i < endCircle; i++) {
        ...
        if (circleInBoxConservative(position.x, position.y, radius, startX, endX, endY, startY)) {
            ...
            deviceSectionCircleLists[sectionNumber * numCircles + i] = true;
        }
    }
    __syncthreads();
    // if (threadId == 0) {
...
CudaRenderer::setup() {
    ...
    cudaCheckError(cudaMemcpyToSymbol(deviceSectionCircleLists, &hostSectionCircleLists, sizeof(bool*)));
